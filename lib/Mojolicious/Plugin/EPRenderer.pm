...
sub register {
  ...
  my $ns = $self->{namespace} = $template->{namespace} =
    defined $template->{namespace} ? $template->{namespace} : 'Mojo::Template::Sandbox::' . md5_sum "$self";
  ...
  $app->renderer->default_handler('ep')->add_handler(
    $conf->{name} || 'ep' => sub {
      ...
      my $name = defined $options->{inline} ? $options->{inline} : $renderer->template_name($options);
